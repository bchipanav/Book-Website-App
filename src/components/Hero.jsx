...
const Hero = ({ searchQuery }) => {
	...
	const getPlaceholder = (title) => {
		const initials = title
			...
			.join("");
		return `data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="600" height="900" viewBox="0 0 600 900"> <rect width="100%" height="100%" fill="#2D3748"/> <text x="50%" y="50%" fill="#4A5568" font-family="monospace" font-size="80" text-anchor="middle" dominant-baseline="middle">${initials}</text></svg>`)}`;
	};
	...
	return (
		<div className="min-h-screen pt-16 md:pt-20 bg-gradient-to-b from-gray-900 to-gray-800p-4 sm:p-6 md:p-8">
					...
					{paginatedBooks.map((book, index) => {
						...
						return (
							<div
								id="hero"
								key={book.id}
								className="group relative bg-gray-800/30 rounded-xl md:rounded-2xl backdrop-blur-xl border border-white/10 hover:border-cyan-400/30 transition-all duration-300 hover:-translate-y-1 md:hover:-translate-y-2 shadow-lg hover:shadow-xl md:shadow-2xl hover:shadow-cyan-400/10"
							>
								...
								<div className="p-4 md:p-6">
									...
									<div className="mt-4 md:mt-6">
										...
										<div className="flex items-center mt-2 md:mt-3">
											<div className="flex stroke-amber-400">
												{[...Array(5)].map((_, i) => (
													<StarIcon
														// biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
														key={i}
														className={`h-4 w-4 md:h-5 md:w-5 ${i < Math.floor(book.rating) ? "fill-amber-400" : "fill-gray-600"}`}
													/>
												))}
											</div>
											...
											{book.categories
												...
												.map((category, index) => (
													...
													>
														{category.trim()}
														{index === 2 &&
															book.categories.split(",").lenght > 3 && (
				...
				<div className="mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-3">
					...
					{generatePageNumber().map((page, index) => (
						<button
							...
							className={`px-3 py-1 md:px-4 md:py-2 rounded-md md:rounded-lg text-xs md:text-sm ${page === currentPage ? "bg-cyan-400/30 text-cyan-300" : "bg-gray-900/80 text-cyan-300 hover:bg-gray-900/60"}`}
						>
							{page}
						</button>
					))}
